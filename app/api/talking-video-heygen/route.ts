...
export async function POST(req: NextRequest) {
    try {
        ...
        const { selfie_public_url: image, voice_public_url: audioUrl, selected_script: script } = sessionData;
        console.log("Audio URL from session:", audioUrl);

        if (!image || !script) return NextResponse.json({ error: 'Missing image or script in session' }, { status: 400 });
        ...
        if (audioUrl) {
            ...
            if (genRes.ok) {
                ...
                if (!id) return NextResponse.json({ error: 'No video id returned', details: genJson, attempt: 'av4-custom-voice' }, { status: 502 });
                return NextResponse.json({ success: true, id, attempt: 'av4-custom-voice' });
            } else {
                return NextResponse.json({ error: 'HeyGen create failed with custom voice', details: { statusCode: genRes.status, body: genParsed.asText } }, { status: 502 });
            }
        } else {
            // Original flow with TTS
            const voice = process.env.HEYGEN_VOICE_ID;
            if (!voice) {
                return NextResponse.json({ error: 'Missing voice_id for TTS. Set HEYGEN_VOICE_ID.' }, { status: 400 });
            }
            ...
            if (genRes.ok) {
                ...
                return NextResponse.json({ success: true, id, attempt: 'av4' });
            }
            ...
            if (genRes.status === 403 && (genJson?.code === 400599 || /requires API Pro plan/i.test(genParsed.asText))) {
                ...
                if (v2Res.ok) {
                    ...
                    return NextResponse.json({ success: true, id, attempt: 'v2-stock-avatar', avatarId });
                }
                return NextResponse.json({ error: 'HeyGen create failed', details: { statusCode: genRes.status, genBodyJson: genJson, genBodyText: genParsed.asText, fallbackStatus: v2Res.status, fallbackBody: v2Parsed.asText } }, { status: 502 });
            }
        }
